...
export function faceGuide(width: number, height: number): Frame {
  ...
  return {
    ...
    y: (height - frameWidth * 1.25) / 2,
    width: frameWidth,
    height: frameWidth * 1.25,
  };
}
...
export function faceInsideGuide(box: Frame, width: number, height: number) {
  ...
  return (
    box.x >= guide.x &&
    box.y >= guide.y &&
    box.x + box.width <= guide.x + guide.width &&
    box.y + box.height <= guide.y + guide.height &&
    box.width >= guide.width * 0.48 &&
    box.height >= guide.height * 0.45
  );
}
...
export function inspectIdFrame(image: ImageData, previous?: Uint8Array) {
    ...
    for (let offset = -band; offset <= band; offset += 1) {
      ...
      for (let step = 0; step < 60; step += 1) {
        ...
        const before = horizontal
          ? sample(along, base + offset - 2)
          : sample(base + offset - 2, along);
  ...
  const edgeScores = {
    top: edge(true, guide.y, guide.x, guide.width),
    bottom: edge(true, guide.y + guide.height, guide.x, guide.width),
    left: edge(false, guide.x, guide.y, guide.height),
    right: edge(false, guide.x + guide.width, guide.y, guide.height),
  };
  const alignedEdges = Object.values(edgeScores).filter(
    (score) => score >= 0.42,
  ).length;
  const edges = alignedEdges === 4;
  let light = 0;
  let sharp = 0;
  let motion = 0;
  let changed = 0;
  ...
  let bright = 0;
  for (
    let y = Math.ceil(guide.y + guide.height * 0.08);
    y < guide.y + guide.height * 0.92;
    y += 2
  ) {
    for (
      let x = Math.ceil(guide.x + guide.width * 0.08);
      x < guide.x + guide.width * 0.92;
      x += 2
    ) {
  ...
  const lit =
    light / count > 45 && light / count < 240 && bright / count < 0.65;
  const crisp = sharp / count > 45;
  const motionScore = motion / count;
  const changedRatio = changed / count;
  const hasPreviousFrame = Boolean(previous?.length === gray.length);
  const stable = hasPreviousFrame && (motionScore < 22 || changedRatio < 0.12);
  const alignmentMessage =
    alignedEdges >= 3
      ? "Center your ID inside the guide."
      : alignedEdges >= 1
        ? "Move farther until all four ID edges are visible."
        : "Move closer and align the ID with the guide.";
  return {
    ...
    message: !lit
      ...
      : !edges
        ? alignmentMessage
        : !crisp
          ? "Move closer and wait for the ID text to become sharp."
          : !stable
            ? "Hold still."
