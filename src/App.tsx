...
export default function App() {
  ...
  const residentProtected = path === '/resident/register' || path === '/resident/census' || path === '/resident/dashboard';
  ...
  if (loading && (residentProtected || adminProtected)) {
    page = <div className="flex min-h-screen items-center justify-center bg-slate-50"><Loader2 className="h-8 w-8 animate-spin text-blue-700"/></div>;
  } else if (residentProtected && !user) {
    page = <ResidentAuth onBack={() => navigate('/')} onLoginSuccess={(destination) => navigate(destination === 'dashboard' ? '/resident/dashboard' : '/resident/census')} onRegisterClick={() => navigate('/resident/register')} />;
  } else if (adminProtected && (!user || !adminProfile || adminProfile.is_active === false)) {
    page = <AdminAuth onBack={() => navigate('/')} onLoginSuccess={() => navigate('/admin/dashboard')} />;
  ...
  return (
    <>
      <InactivityTimer />
      {logoutError && <div role="alert" className="fixed bottom-4 left-1/2 z-[300] -translate-x-1/2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700 shadow-lg">{logoutError}</div>}
      {page}
    </>
  );
}
