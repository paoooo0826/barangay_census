...
export default function ResidentRegistration({
  ...
  function replaceImage(
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Unable to use that image.",
      );
    }
  }
  ...
  async function saveVerification(
    ...
    try {
      ...
      if (existing)
        throw new Error(
          "This account already has a resident census record. Open your dashboard instead.",
        );

      const folder = `${user.id}/${crypto.randomUUID()}`;
      const frontImagePath = await uploadFile(
        frontImage,
      ...
      sessionStorage.setItem(
        `pendingResidentVerification:${user.id}`,
  ...
  function imageField(
    ...
    return (
      <div
        className={`rounded-2xl border-2 p-4 ${fieldErrors[field] ? "border-red-400 bg-red-50" : "border-slate-200 bg-slate-50"}`}
      >
        <div className="mb-3 flex items-center justify-between gap-2">
          ...
          {image && (
            <button
              type="button"
              onClick={() => removeImage(preview, setImage, setPreview)}
              className="rounded-lg p-1 text-slate-500 hover:bg-white hover:text-red-600"
        ...
        {preview ? (
          <img
            src={preview}
            alt={`${title} preview`}
            className="h-52 w-full rounded-xl bg-white object-contain"
          />
        ) : (
          <div className="flex h-36 items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white text-slate-400">
            <FileImage size={38} />
          </div>
        )}
          ...
          <label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">
            ...
            <input
              ...
              onChange={(event) => {
                ...
                if (file)
                  replaceImage(file, preview, setImage, setPreview, field);
                event.currentTarget.value = "";
              }}
            />
          </label>
          <IdCameraCapture
            side={field === "frontImage" ? "front" : "back"}
            autoOpen={
              field === "frontImage"
                ? Boolean(idType && !frontImage)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-4xl">
        <button
          ...
          className="mb-5 inline-flex items-center gap-2 font-semibold text-slate-600 hover:text-slate-900"
        >
          <ArrowLeft size={19} />
          Back
        </button>
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl">
          <div className="bg-gradient-to-r from-blue-800 to-blue-600 px-6 py-7 text-white sm:px-8">
            ...
            <div className="mt-6 grid grid-cols-3 gap-2 text-center text-xs font-bold sm:text-sm">
              {["Government ID", "Live verification", "Complete"].map(
                (label, index) => (
                  <div
                    key={label}
            ...
            {step === 3 && complete && (
              <div className="py-8 text-center">
                ...
                <p className="mx-auto mt-2 max-w-lg text-slate-600">
                  Continue to the census form. Your final identity and census
                  information will still be available for administrator
                  checking.
                </p>
                <button
                  type="button"
