...
interface ResidentImages { household:string|null; idFront:string|null; idBack:string|null; face:string|null; }
...
export default function ResidentDashboard({ onLogout, onEdit }: Props) {
  ...
  const [images,setImages]=useState<ResidentImages>({household:null,idFront:null,idBack:null,face:null});
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 text-slate-900">
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
      ...
      {tab==='profile'&&<Profile resident={resident} accountEmail={user?.email??''} categories={categories} governmentIdType={governmentIdType} images={images} onOpenImage={(title,url)=>setPreviewImage({title,url})} onEdit={onEdit}/>} 
...
function Profile({resident,accountEmail,categories,governmentIdType,images,onOpenImage,onEdit}:{resident:Resident|null;accountEmail:string;categories:ResidentCategoryView[];governmentIdType:string|null;images:ResidentImages;onOpenImage:(title:string,url:string)=>void;onEdit:()=>void}) {
  if (!resident) return <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm"><h2 className="text-2xl font-bold">Resident Profile</h2><p className="mt-2 text-slate-500">Complete your census form to create your resident profile.</p><button onClick={onEdit} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-blue-700 px-5 py-3 font-bold text-white"><Edit3 size={18}/>Complete Census Form</button></section>;
  const categoryText=categories.length?categories.map((category)=>[category.name,category.indigenous_group,category.other_description].filter(Boolean).join(': ')).join(', '):'None selected';
  return <section className="space-y-6">
    <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-8"><div><p className="text-sm font-semibold text-blue-700">Account & Census Record</p><h2 className="mt-1 text-2xl font-bold">Resident Profile</h2><p className="mt-1 text-sm text-slate-500">All information saved in your census record is shown below. Your account email remains protected.</p></div><button onClick={onEdit} className="inline-flex items-center gap-2 rounded-xl bg-blue-700 px-5 py-3 font-bold text-white"><Edit3 size={18}/>Update Information</button></div>
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Info label="Account Email (Locked)" value={accountEmail}/><Info label="Tracking Number" value={resident.tracking_number}/><Info label="Census Status" value={STATUS_CONFIG[resident.status].label}/><Info label="Date Submitted" value={formatDate(resident.submitted_at)}/></div>
    <div className="grid gap-6 lg:grid-cols-2">
      <SummaryCard title="Personal Information" rows={[["First Name",resident.first_name],["Middle Name",resident.middle_name],["Last Name",resident.last_name],["Suffix",resident.suffix],["Birth Date",formatDate(resident.birth_date)],["Birth Place",resident.birth_place],["Sex",resident.sex],["Civil Status",resident.civil_status],["Religion",resident.religion],["Citizenship",resident.citizenship]]}/>
      <SummaryCard title="Contact & Address" rows={[["Region",resident.region],["Province",resident.province],["City / Municipality",resident.city_municipality],["Barangay",resident.barangay],["Residential Address",resident.residential_address],["Contact Number",resident.contact_number],["Email Address",accountEmail],["PhilSys Number",resident.philsys_number]]}/>
      <SummaryCard title="Education & Occupation" rows={[["Highest Education",resident.highest_education],["Education Status",resident.education_status],["Vocational Course",resident.vocational_course],["Profession / Occupation",resident.profession_occupation]]}/>
      <SummaryCard title="Housing & Categories" rows={[["Tenurial Status",resident.tenurial_status],["Monthly Rent",resident.monthly_rent!=null?`₱${Number(resident.monthly_rent).toLocaleString('en-PH')}`:null],["Resident Categories",categoryText],["Last Updated",formatDate(resident.updated_at)]]}/>
    </div>
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-lg font-bold">Submitted Verification Photos</h3><p className="mt-1 text-sm text-slate-500">The identification and household photos connected to your record.</p></div><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">ID type: {display(governmentIdType)}</span></div><div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><ImageCard title="Household Photo" url={images.household} onOpen={(url)=>onOpenImage('Household Photo',url)}/><ImageCard title="Government ID Front" url={images.idFront} onOpen={(url)=>onOpenImage('Government ID Front',url)}/><ImageCard title="Government ID Back" url={images.idBack} onOpen={(url)=>onOpenImage('Government ID Back',url)}/><ImageCard title="Captured Live Face" url={images.face} onOpen={(url)=>onOpenImage('Captured Live Face',url)}/></div></div>
  </section>;
}
